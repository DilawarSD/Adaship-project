...
bool Ships::isValidPlacement(int startRow, int startCol, int size,
                             bool isHorizontal,
                             const std::vector<std::vector<char>> &board) {
  // Check if the placement is within the bounds of the board
  if (startRow < 0 || startRow >= board.size() || startCol < 0 ||
...
void Ships::manualPlaceShip(const Boat &boat, const Config &config,
  ...
  while (true) {
    ...
    if (userInput.size() < 2 || !std::isalpha(userInput[0]) ||
        !std::isdigit(userInput[1])) {
      std::cout
          << "Invalid input format. Please enter a valid starting position."
          << std::endl;
      continue; // Go to the next iteration of the loop to allow the user to
                // re-enter
    }

    char userInputColumn =
        toupper(userInput[0]); // Convert to uppercase for consistency
    int userInputRow =
        userInput[1] - '0'; // Convert the second character to an integer
    ...
    if ((orientation == 'H' || orientation == 'h') &&
        isValidPlacement(startRowIndex, startColIndex, size, true, board)) {
      // Update the board for horizontal placement
      for (int i = 0; i < size; ++i) {
        board[startRowIndex][startColIndex + i] =
            'S'; // Use 'S' to represent the ship
      }
      break; // Exit the loop if placement is valid
    } else if ((orientation == 'V' || orientation == 'v') &&
               isValidPlacement(startRowIndex, startColIndex, size, false,
                                board)) {
      // Update the board for vertical placement
      for (int i = 0; i < size; ++i) {
        board[startRowIndex + i][startColIndex] =
            'S'; // Use 'S' to represent the ship
      }
      break; // Exit the loop if placement is valid
    } else {
...
void Ships::autoPlaceShip(const Boat &boat, const Config &config,
                          std::vector<std::vector<char>> &board) {
  int size = boat.size;
...
void Ships::manualPlaceAllShips(const std::vector<Boat> &boats,
                                const Config &config,
                                std::vector<std::vector<char>> &board) {
  if (isPlayer) {
    ...
    do {
      ...
      // Validate the input
      while (toupper(manualPlaceChoice) != 'Y' &&
             toupper(manualPlaceChoice) != 'N') {
      ...
      if (toupper(manualPlaceChoice) == 'Y') {
        for (size_t i = 0; i < boats.size(); ++i) {
          // Ask if the user wants to place the next ship (skip for the first
          // iteration)
          if (i > 0) {
...
bool Ships::isShipPlaced(const Boat &boat,
                         const std::vector<std::vector<char>> &board) {
  ...
  return count >= shipSize; // Ship is considered placed if at least its size
                            // number of cells are found
}

bool Ships::allShipPlaced(const std::vector<Boat> &boats,
                          const std::vector<std::vector<char>> &board) {
