...
void Player::playerMove(std::vector<std::vector<char>> &board) {
  ...
  while (true) {
    ...
    if (std::toupper(choice) == 'M') {
      ...
      if (!(std::cin >> move) || move.size() < 2 || !isalpha(move[0]) ||
          !isdigit(move[1])) {
        std::cin.clear();
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        std::cout << "Invalid input. Please enter a valid move in the format "
                     "'A1' or 'F6'."
                  << std::endl;
        continue;
      }
      ...
      std::cout << "Auto-firing at cell " << static_cast<char>('A' + col)
                << row + 1 << "..." << std::endl;
    } else {
      std::cout << "Invalid choice. Please enter 'M' for manual input or 'A' "
                   "for auto-fire."
                << std::endl;
      continue;
    }
    ...
    if (board[row][col] == 'X' || board[row][col] == 'O') {
      std::cout << "Invalid move. Cell has already been hit. Please try again."
                << std::endl;
      continue;
    }
